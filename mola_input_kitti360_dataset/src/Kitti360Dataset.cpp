...
std::vector<double> parseKitti360Timestamps(const std::string& fil)
{
  ...
  while (parser.getNextLine(line))
  {
    ...
    int nParsedFields = ::sscanf(
        line.c_str(), "%04u-%02u-%02u %02u:%02u:%lf", &year, &month, &day, &hour, &min, &sec);
...
void Kitti360Dataset::initialize_rds(const Yaml& c)
{
  ...
  const std::string regExpr  = "(00|02|03|04|05|06|07|08|09|10|18|test_0|test_1|test_2|test_3)";
  ...
  {
    THROW_EXCEPTION_FMT(
        "Error: sequence is '%s' but must be one of '%s'", sequence_.c_str(), regExpr.c_str());
  }
  ...
  {
    const std::string syncDir = mrpt::format("2013_05_28_drive_00%s_sync", sequence_.c_str());

    lidar_dir = mrpt::system::pathJoin({base_dir_, "data_3d_raw", syncDir, "velodyne_points"});
    MRPT_LOG_DEBUG_STREAM("Expected LIDAR dir: " << lidar_dir);

    for (int i = 0; i < 4; i++)
    {
      images_dirs[i] = mrpt::system::pathJoin(
          {base_dir_, "data_2d_raw", syncDir, mrpt::format("image_%02i", i)});

      MRPT_LOG_DEBUG_STREAM("Expected images dir: " << images_dirs[i]);
    }

    gt_file = mrpt::system::pathJoin({base_dir_, "data_3d_raw", syncDir, "poses.txt"});
  ...
  {
    MRPT_LOG_WARN_STREAM("Ground truth poses: not found. Expected file: " << gt_file);
    gt_file.clear();
  }
  ...
  MRPT_LOG_INFO_STREAM(
      ...
      << (!lst_velodyne_.empty() ? "Found ("s + std::to_string(lst_velodyne_.size()) + ")"s
                                 : "Not found"));
  ...
  const std::string filCalibCamToPose =
      mrpt::system::pathJoin({base_dir_, "calibration", "calib_cam_to_pose.txt"});
  const std::string filCalibCamToVelo =
      mrpt::system::pathJoin({base_dir_, "calibration", "calib_cam_to_velo.txt"});
  ...
  for (int i = 0; i < 2; i++)
  {
    parse_calib_line(calib[mrpt::format("P_rect_%02i", i)].as<std::string>(), P01[i]);
    parse_calib_line(calib[mrpt::format("S_rect_%02i", i)].as<std::string>(), S01[i]);
  ...
  const auto T_veh_gps = mrpt::poses::CPose3D::FromYawPitchRoll(0.0_deg, 0.0_deg, 180.0_deg);

  // Camera intrinsics:
  for (unsigned int i = 0; i < 2; i++)
  {
    const double fx = P01[i](0, 0), fy = P01[i](1, 1), cx = P01[i](0, 2), cy = P01[i](1, 2);
  ...
  const auto yamlCamToPose = mrpt::containers::yaml::FromFile(filCalibCamToPose);
  for (int i = 0; i < 4; i++)
  {
    Eigen::Matrix4d T  = Eigen::Matrix4d::Identity();
    auto            Tr = Eigen::Matrix<double, 3, 4>();
    parse_calib_line(yamlCamToPose[mrpt::format("image_%02i", i)].as<std::string>(), Tr);
    T.block<3, 4>(0, 0) = Tr;
  ...
  // velodyne -> image_00:
  {
    ...
    const auto cam00_to_velo = mrpt::poses::CPose3D::FromHomogeneousMatrix(T_cam00_to_velo);
  ...
  if (!gt_file.empty())
  {
    ...
    for (int i = 0; i < M.rows(); i++)
    {
      ...
      groundTruthTrajectory_.insert(mrpt::Clock::fromDouble(lstLidarTimestamps_.at(idx)), gtPose);
    }
...
void Kitti360Dataset::spinOnce()
{
  ...
  while (replay_next_tim_index_ < lstLidarTimestamps_.size() &&
         last_dataset_time_ >= (lstLidarTimestamps_[replay_next_tim_index_] - t0))
  {
    ...
    const auto obs_tim = mrpt::Clock::fromDouble(lstLidarTimestamps_[replay_next_tim_index_]);
    ...
    if (publish_ground_truth_ && replay_next_tim_index_ < groundTruthTrajectory_.size())
    {
...
void Kitti360Dataset::load_img(const unsigned int cam_idx, const timestep_t step) const
{
  MRPT_START
  ...
  const auto f = mrpt::system::pathJoin({lst_image_basedir_[cam_idx], lst_image_[cam_idx][step]});
...
void Kitti360Dataset::load_lidar(timestep_t step) const
{
  ...
  const auto f = mrpt::system::pathJoin({lst_velodyne_basedir_, lst_velodyne_[step]});
  ...
  if (VERTICAL_ANGLE_OFFSET != 0)
  {
    ...
    for (size_t i = 0; i < xs.size(); i++)
    {
      ...
      const auto aa = Eigen::AngleAxisd(VERTICAL_ANGLE_OFFSET, rotationVector.normalized());
...
mrpt::obs::CObservation::Ptr Kitti360Dataset::getPointCloud(timestep_t step) const
{
  ASSERT_(initialized_);
...
mrpt::obs::CSensoryFrame::Ptr Kitti360Dataset::datasetGetObservations(size_t timestep) const
{
  {
    auto lck             = mrpt::lockHelper(dataset_ui_mtx_);
