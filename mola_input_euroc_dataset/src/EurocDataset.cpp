...
void EurocDataset::initialize_rds(const Yaml& c)
{
  ...
  for (uint8_t cam_id = 0; cam_id < 2; cam_id++)
  {
    ...
    const auto cam_data_fil = seq_dir_ + "/cam"s + std::to_string(cam_id) + "/data.csv"s;
    ...
    for (int row = 0; row < dat.rows(); row++)
    {
      ...
      se_cam.img_file_name = "/cam"s + std::to_string(cam_id) + "/data/"s +
                             std::to_string(static_cast<euroc_timestamp_t>(dat(row, 1))) + ".png"s;

      dataset_.emplace_hint(dataset_.end(), t, se_cam);
    }

    MRPT_LOG_INFO_STREAM(
        "cam" << std::to_string(cam_id) << ": Loaded " << dat.rows() << " entries.");

    // Load calibration:
    const auto fil_calib = seq_dir_ + "/cam"s + std::to_string(cam_id) + "/sensor.yaml"s;
    ...
    const mrpt::poses::CPose3D imu2veh(
        0, 0, 0, mrpt::DEG2RAD(180.0), mrpt::DEG2RAD(-90.0), mrpt::DEG2RAD(0.0));

    mrpt::math::CMatrixDouble44 HM;

    auto itN = cam_pose.asSequence().begin();
    for (int r = 0; r < 4; r++)
      for (int col = 0; col < 4; col++, ++itN) HM(r, col) = itN->as<double>();
    ...
    ASSERT_(cal["distortion_model"].as<std::string>() == "radial-tangential");
...
void EurocDataset::spinOnce()
{
  ...
  const euroc_timestamp_t tim =
      static_cast<euroc_timestamp_t>(last_dataset_time_ * 1e9) + dataset_.begin()->first;
  ...
  else
  {
    MRPT_LOG_THROTTLE_INFO_FMT(
        ...
        static_cast<unsigned long>(dataset_cur_idx_), static_cast<unsigned long>(dataset_.size()),
  ...
  while (dataset_next_ != dataset_.end() && tim >= dataset_next_->first)
  {
    ...
    const auto obs_tim = mrpt::Clock::fromDouble(dataset_next_->first * 1e-9);

    std::visit(
        overloaded{
            [&](std::monostate&) { THROW_EXCEPTION("Un-initialized entry!"); },
            [&](SensorCamera& cam)
            {
  ...
  // Read ahead to save delays in the next iteration:
  {
    ...
    ++peeker;
    for (unsigned int i = 0; i < READ_AHEAD_COUNT && peeker != dataset_.end(); ++i, ++peeker)
    {
      //
      std::visit(
          overloaded{
