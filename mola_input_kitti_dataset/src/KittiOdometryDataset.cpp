...
void KittiOdometryDataset::initialize_rds(const Yaml& c)
{
  ...
  time_warp_scale_ = cfg.getOrDefault<double>("time_warp_scale", time_warp_scale_);
  ...
  for (unsigned int i = 0; i < 4; i++)
  {
    parse_calib_line(calib["P" + std::to_string(i)].as<std::string>(), P[i]);
  }
  ...
  if (mrpt::system::fileExists(gtFile))
  {
    ...
    ASSERT_EQUAL_(static_cast<size_t>(groundTruthPoses_.rows()), lst_timestamps_.size());
    ...
    const auto axisChange = mrpt::poses::CPose3D::FromYawPitchRoll(-90.0_deg, 0.0_deg, -90.0_deg);
    ...
    for (size_t i = 0; i < lst_timestamps_.size(); i++)
    {
      ...
      groundTruthTrajectory_.insert(mrpt::Clock::fromDouble(lst_timestamps_.at(i)), gtPose);
    }
...
void KittiOdometryDataset::load_img(const unsigned int cam_idx, const timestep_t step) const
{
  MRPT_START
  ...
  const auto f = seq_dir_ + std::string("/image_") + std::to_string(cam_idx) + std::string("/") +
                 lst_image_[cam_idx][step];
...
mrpt::obs::CObservation::Ptr KittiOdometryDataset::getPointCloud(timestep_t step) const
{
  ASSERT_(initialized_);
...
mrpt::obs::CSensoryFrame::Ptr KittiOdometryDataset::datasetGetObservations(size_t timestep) const
{
  {
    auto lck             = mrpt::lockHelper(dataset_ui_mtx_);
