...
void ParisLucoDataset::initialize_rds(const Yaml& c)
{
  ...
  build_list_files(mrpt::system::pathJoin({seq_dir_, "frames"}), "ply", lstLidarFiles_);

  MRPT_LOG_INFO_STREAM(
      "LIDAR scans: "
      << (!lstLidarFiles_.empty() ? "Found ("s + std::to_string(lstLidarFiles_.size()) + ")"s
                                  : "Not found"));
  ...
  lst_timestamps_.resize(lstLidarFiles_.size());
  {
    double t = 0;
    std::generate(
        lst_timestamps_.begin(), lst_timestamps_.end(), [&t, this] { return t += lidarPeriod_; });
  }
  ...
  if (mrpt::system::fileExists(gtFile))
  {
    ...
    ASSERT_EQUAL_(static_cast<size_t>(groundTruthTranslations_.rows()), lstLidarFiles_.size());

    MRPT_LOG_INFO("Ground truth translations: Found");
  }
  else
  {
    MRPT_LOG_WARN_STREAM("Ground truth translations: not found. Expected file: " << gtFile);
  }
...
mrpt::obs::CSensoryFrame::Ptr ParisLucoDataset::datasetGetObservations(size_t timestep) const
{
  ASSERT_(initialized_);
